...
export default function AddKamerad() {
  ...
  const handlePublish = () => {
    if (!formData.judul || !formData.desc || !formData.date || !formData.image ) {
      toast("Please fill all the fields");
      return;
    }

    const storageRef = ref(storage, `/studentchapter/${formData.image.name}`);
      ...
      () => {
        ...
        getDownloadURL(uploadImage.snapshot.ref).then((url) => {
          ...
          addDoc(kameradRef, {
            ...
            .then(() => {
              toast("student chapter lahir", { type: "success" });
              setProgress(0);
            })
            .catch((err) => {
